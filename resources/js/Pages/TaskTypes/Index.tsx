...
interface IndexProps {
    types: PaginatedResponse<TaskType>;
    filters: SearchFilter;
}

export default function Index({ types, filters }: IndexProps) {
    const [editingType, setEditingType] = useState<TaskType | null>(null);
    const [isCreating, setIsCreating] = useState(false);
    const [searchQuery, setSearchQuery] = useState(filters.search);

    const { data, setData, post, put, delete: destroy, errors, reset, processing } = useForm<TaskTypeFormData>({
        name: '',
    });

    const handleSearch = (value: string) => {
        setSearchQuery(value);
        router.get(route('admin.task-types.index'), { search: value }, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingType) {
            put(route('admin.task-types.update', editingType.id), {
                onSuccess: () => {
                    setEditingType(null);
                    reset();
                },
            ...
            post(route('admin.task-types.store'), {
                onSuccess: () => {
                    reset();
                    setIsCreating(false);
                },
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this task type?')) {
            destroy(route('admin.task-types.destroy', id), {
                onSuccess: () => {
                                    ...
                                    <CardTitle className="text-white text-lg flex items-center gap-2">
                                        {editingType ? (
                                            ...
                                            <>
                                                ...
                                                {isCreating ? 'Create New Type' : 'Add Type'}
                                            </>
                                        )}
                                    </CardTitle>
                                </CardHeader>
                                    ...
                                    {types.data.length === 0 ? (
                                        <div className="text-center py-12">
                                            ...
                                            <p className="text-slate-500 text-lg">
                                                {searchQuery ? 'No types found' : 'No types yet'}
                                            </p>
                                            <p className="text-slate-600 text-sm mt-1">
                                                {searchQuery ? 'Try a different search term' : 'Create your first type to get started'}
                                            </p>
                                        </div>
                                    ) : (
                                        <>
                                                        ...
                                                        {types.data.map((type) => (
                                                            <tr
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    editingType?.id === type.id && "bg-indigo-500/10"
                                                                )}
                                                            >
                                                ...
                                                <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-800">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <Button
                                                            variant="outline"
                                                            size="sm"
                                                            onClick={() => router.get(route('admin.task-types.index'), { page: types.current_page - 1, search: searchQuery })}
                                                            disabled={types.current_page === 1}
                                                            className="border-slate-700 text-slate-300 hover:bg-slate-800/50 hover:text-white"
                                                        >
                                                            Previous
                                                        </Button>
                                                        <span className="text-sm text-slate-400">
                                                            Page {types.current_page} of {types.last_page}
                                                        </span>
                                                        <Button
                                                            variant="outline"
                                                            size="sm"
                                                            onClick={() => router.get(route('admin.task-types.index'), { page: types.current_page + 1, search: searchQuery })}
                                                            disabled={types.current_page === types.last_page}
                                                            className="border-slate-700 text-slate-300 hover:bg-slate-800/50 hover:text-white"
                                                        >
                                                            Next
                                                        </Button>
